    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        const formattedReviews = reviewsData.map(review => ({
          ...review,
          formattedDate: format(new Date(review.created_at), 'dd/MM/yyyy HH:mm')
        }));
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <div className="review-preview">
              <div><strong>Phòng:</strong> {selectedReview?.room_name}</div>
              <div><strong>Người dùng:</strong> {selectedReview?.user_name}</div>
              <div><strong>Đánh giá:</strong> {selectedReview?.rating}/5</div>
              <div><strong>Nội dung:</strong> {selectedReview?.comment}</div>
            </div>
            <div className="modal-actions">
              <button className="cancel-button" onClick={closeStatusModal}>Hủy</button>
              <button 
                className={newStatus === 'approved' ? 'approve-button' : 'reject-button'} 
